...
// Serious commands BEFORE MODIFICATION
/*
...
public plugin_init()
{
	...
	g_pAllowPublicSpec 		= register_cvar("allow_public_spec","0");
	...
	register_concmd("amx_alltalk", 	"Cmd_AllTalk", 	RANK_OWNER, "[1 = ON | 0 = OFF]");
	register_concmd("amx_extend", 	"Cmd_Extend", 	RANK_OWNER, "<added time to extend> : ex. 5, if you want to extend it five more minutes.");
	register_concmd("amx_gag", 	"Cmd_Gag", 	RANK_ADMIN, "<nick, #userid or authid> <a|b|c> <time> - Flags: a = Normal Chat | b = Team Chat | c = Voicecomm");
	register_concmd("amx_ungag", 	"Cmd_Ungag", 	RANK_DEPUTY_HEAD, "<nick, #userid or authid>");
	register_concmd("amx_pass", 	"Cmd_Pass", 	RANK_OWNER,"<password> - sets the server's password")
	register_concmd("amx_nopass", 	"Cmd_NoPass", 	RANK_OWNER,"Removes the server's password")
	register_concmd("amx_transfer", "Cmd_Transfer",	RANK_DEPUTY_HEAD, "<nick, #userid or authid> <CT/T/Spec> Transfers that player to the specified team");
	register_concmd("amx_swap", 	"Cmd_Swap", 	RANK_HEAD_ADMIN, "<nick, #userid or authid> <nick, #userid or authid> Swaps 2 players with eachother");
	register_concmd("amx_teamswap", "Cmd_TeamSwap",	RANK_ADMINISTRATOR, "Swaps 2 teams with eachother");
	register_concmd("amx_lock", 	"Cmd_Lock", 	RANK_CO_OWNER, "<CT/T/Auto/Spec> - Locks selected team");
	register_concmd("amx_unlock",	"Cmd_Unlock",	RANK_CO_OWNER, "<CT/T/Auto/Spec> - Unlocks selected team");
	register_concmd("amx_badaim", 	"Cmd_BadAim", 	RANK_OWNER, "<nick, #userid or authid> <On/off or length of time: 1|0|time> <Save?: 1|0>: Turn on/off bad aim on a player.");
	register_concmd("amx_exec", 	"Cmd_Exec", 	RANK_OWNER, "<nick, #userid, authid or @team> <command>");
	register_concmd("amx_restart", 	"Cmd_Restart", 	RANK_OWNER, 	   "<seconds (1-20)> - Restarts the server in seconds");
	register_concmd("amx_shutdown", "Cmd_Restart", 	RANK_OWNER,    "<seconds (1-20)> - Shuts down the server in seconds");
	...
	// Team Locker
	RegisterHookChain(RG_CBasePlayer_CanSwitchTeam, "Player_CanSwitchTeam_Pre", false);
...
public Cmd_Gag(id, iLevel, iCid)
{
	...
	new iPlayer = cmd_target(id, szTarget, CMDTARGET_ALLOW_SELF | CMDTARGET_NO_BOTS);
	
	if(iPlayer)
	{
		// here player can affect himself and lower ranks only!
		if (id != iPlayer && !canAffect(id, iPlayer))
		{
			new szName[35], szRank[35];
			get_user_name(iPlayer, szName, charsmax(szName));
			GetRankLevelStr(iPlayer, szRank, charsmax(szRank));
			console_print(id, "%L", id, "AMX_SUPER_CANT_AFFECT", szName, szRank);
			return PLUGIN_HANDLED;
		}
...
public Cmd_Ungag(id, iLevel, iCid)
{
	...
	if(iPlayer)
	{
		...
		if (!canAffect2(id, iPlayer))
		{
			new szRank[35];
			GetRankLevelStr(iPlayer, szRank, charsmax(szRank));
			console_print(id, "%L", id, "AMX_SUPER_CANT_AFFECT", szPlayerName, szRank);
			return PLUGIN_HANDLED;
		}
...
public Cmd_Transfer(id, iLevel, iCid)
{
	...
	new iTempid = cmd_target(id, szTarget, CMDTARGET_ALLOW_SELF);
	new szPlayerName[35];
	...
	new szTeamName[35];
	new iCurrentTeam = GetTeam(iTempid)
	...
	{
		new TeamName:iArgTeam
		switch(szTeam[0])
		{
			case 'C': iArgTeam = TEAM_CT;
			case 'T': iArgTeam = TEAM_TERRORIST;
			case 'S': iArgTeam = TEAM_SPECTATOR;
			default:
			{
				console_print(id, "%L", id, "AMX_SUPER_TEAM_INVALID");
				return PLUGIN_HANDLED;
		...
		else
		{
			...
			*/
			new bool:CheckWinConditions = (iArgTeam == TEAM_SPECTATOR);
			rg_set_user_team(iTempid, iArgTeam, MODEL_AUTO, true, CheckWinConditions);
			
			if(iArgTeam == TEAM_SPECTATOR)
				user_silentkill(iTempid);
				
			else
				rg_round_respawn(iTempid);

			// using teamnames variable if szTeam != spec
			formatex(szTeamName, charsmax(szTeamName), "%s", iArgTeam == TEAM_SPECTATOR ? "Spectator" : Teamnames[_:iArgTeam - 1])
		}
	}
...
public Cmd_Swap(id, iLevel, iCid)
{
	...
	new iTempid1 = cmd_target(id, szArg1, CMDTARGET_ALLOW_SELF);
	new iTempid2 = cmd_target(id, szArg2, CMDTARGET_ALLOW_SELF);
	
	if(!iTempid1 || !iTempid2)
		return PLUGIN_HANDLED;
		
	new szPlayerName1[35], szPlayerName2[35];
	get_user_name(iTempid1, szPlayerName1, charsmax(szPlayerName1));
	get_user_name(iTempid2, szPlayerName2, charsmax(szPlayerName2));
	
	if (!canAffect2(id, iTempid1))
	{
		new szRank[35];
		GetRankLevelStr(iTempid1, szRank, charsmax(szRank));
		console_print(id, "%L", id, "AMX_SUPER_CANT_AFFECT", szPlayerName1, szRank);
		return PLUGIN_HANDLED;
	}

	if (!canAffect2(id, iTempid2))
	{
		new szRank[35];
		GetRankLevelStr(iTempid2, szRank, charsmax(szRank));
		console_print(id, "%L", id, "AMX_SUPER_CANT_AFFECT", szPlayerName2, szRank);
		return PLUGIN_HANDLED;
	}
	
	new iTeam1 = GetTeam(iTempid1);
	new iTeam2 = GetTeam(iTempid2);
	...
	if(iTeam1 == UNASSIGNED || iTeam2 == UNASSIGNED)
	{
	...
	rg_set_user_team(iTempid1, TeamName:iTeam2, true);
	rg_set_user_team(iTempid2, TeamName:iTeam1, true);
	
	if(iTeam1 == SPECTATOR)
		user_silentkill(iTempid2);
	else
		rg_round_respawn(iTempid1);
	
	if(iTeam2 == SPECTATOR)
		user_silentkill(iTempid1);
	else
		rg_round_respawn(iTempid2);
		
	new szAdminName[35], szAuthid[35];
...
public Cmd_TeamSwap(id, iLevel, iCid)
{
	...
	get_players(iPlayers, iPlayerNum, "h");
	
	for(new i = 0; i < iPlayerNum; i++)
	{
		iTempid = iPlayers[i];
		if (!mjb_is_valid_player(iTempid))
			continue;
		if (!canAffect2(id, iTempid))
			continue;
		iTeam = GetTeam(iTempid);
		
		if(UNASSIGNED < iTeam < SPECTATOR)
		{
			rg_set_user_team(iTempid, TeamName:iTeam == TEAM_TERRORIST ? TEAM_CT : TEAM_TERRORIST, true);
			rg_round_respawn(iTempid);
		}
	}
...
public Cmd_Lock(id, iLevel, iCid)
{
	...
	switch(szArg[0])
	{
		case 'T': iTeam = PRISONER;
		case 'C': iTeam = GUARD;	
		case 'S': iTeam = SPECTATOR;
		default:
		{	
 ...
 *----------------
*/
public Player_CanSwitchTeam_Pre(id, TeamName:teamToSwap)
{
	if (GetTeam(id) != UNASSIGNED && g_bBlockJoin[_:teamToSwap] && !hasRank(id, RANK_CO_OWNER)) {
		client_print(id, print_chat, "%L", id, "AMX_SUPER_TRYING_TEAM_LOCKED");
...
public Cmd_Exec(id, iLevel, iCid)
{
	...
	if(szTarget[0] == '@')
	{
		...
		{
			iPlayer = iPlayers[i];
			if (!canAffect(id, iPlayer) && iPlayer != id)
			{
				new szRank[35];
				get_user_name(iPlayer, szTargetName, charsmax(szTargetName));
				GetRankLevelStr(iPlayer, szRank, charsmax(szRank));
				console_print(id, "%L", id, "AMX_SUPER_TEAM_IMMUNITY", szTargetName, szRank);
				continue;
			}
			...
			else if(bIsFilteredCmd && !g_bAllowsAllCommands[iPlayer])
			{
				// TODO: Add ML support.
				get_user_name(iPlayer, szTargetName, charsmax(szTargetName));
				console_print(id, "The command %s is banned and does not work on Players.", szCommand);
				continue;
			}
...
public Cmd_Spec(id)
{
	...
	if(( Team == PRISONER || Team == GUARD ) && get_pcvar_num(g_pAllowSpec) || get_pcvar_num(g_pAllowPublicSpec)) 
	{
		if(is_user_alive(id))
		{
			user_kill(id, false);
...
public Cmd_UnSpec(id)
{
	...
	if(g_OldTeam[id])
	{
		rg_set_user_team(id, TeamName:g_OldTeam[id], MODEL_AUTO, true, true);
		g_OldTeam[id] = UNASSIGNED;
	}
...
public Cmd_Admins(id) 
{
	...
	if(get_pcvar_num(g_pAdminCheck))
	{
		...
		{
			if(is_user_connected(i) && (get_user_flags(i) & RANK_ADMIN))
