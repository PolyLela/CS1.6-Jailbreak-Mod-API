...
Handle_Message(id, szArgs[CC_MAX_MESSAGE_SIZE], bool:bTeam)
{
	...
	{
		for(new bool:bSend, iPlayerTeam, iPlayer, i; i < iPnum; i++)
		{
			bSend = false
			iPlayer = iPlayers[i]

			iPlayerTeam = GetTeam(iPlayer)
...
apply_replacements(const szFormat[], id, iAlive, iTeam, const szArgs[], szMessage[], iLen)
{
	...
	replace_string(szMessage, iLen, ARG_DEAD_PREFIX, g_eSettings[iAlive ? ALIVE_PREFIX : iTeam == SPECTATOR ? SPEC_PREFIX : DEAD_PREFIX])
	#endif

	#if defined ARG_TEAM
	replace_string(szMessage, iLen, ARG_TEAM, g_eSettings[iTeam == GUARD ? TEAM_PREFIX_CT : iTeam == PRISONER ? TEAM_PREFIX_T : TEAM_PREFIX_SPEC])
	#endif
