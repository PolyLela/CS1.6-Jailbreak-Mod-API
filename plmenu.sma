...
public plugin_init()
{
	...
	register_clcmd("amx_kickmenu", "cmdKickMenu", RANK_ADMIN, "- displays kick menu");
	register_clcmd("amx_banmenu", "cmdBanMenu", RANK_FULL_ADMIN, "- displays ban menu");
...
displayBanMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (is_user_bot(i) || !canAffect(id, i))
		{
			++b;
...
displaySlapMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (!is_user_alive(i) || !canAffect(id, i))
		{
			++b;
...
displayTeamMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if ((iteam == g_CSTeamiNumbers[g_menuOption[id] % 3]) || !canAffect(id, i))
		{
			++b;
...
displayClcmdMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (!g_menuSelectNum[id] || !canAffect(id, i))
		{
			++b;
