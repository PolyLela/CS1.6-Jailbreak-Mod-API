...
GetMenuPlayers(&iNum, iMenu)
{
	...
	switch(iMenu)
	{
		case FIRE, FLASH, DISARM, ROCKET, UBERSLAP, DRUG, HEAL, ARMOR, STACK, BURY, UNBURY, SLAY, INVINCIBLE, NOCLIP, SPEED, UNAMMO, BADAIM:
...
public MainMenuHandler(id, hMenu, iItem)
{
	...
	switch(iNum)
	{
		...
		case HEAL, ARMOR, STACK, BURY, UNBURY, SLAY, INVINCIBLE, NOCLIP, SPEED, UNAMMO, SETMONEY, BADAIM:
	...
	switch (iKey)
	{
		case 7:
		{
			switch (g_iMenu[id])
			{
				...
				case STACK, INVINCIBLE, NOCLIP: if (++g_iMenuProperties[id] > 2) g_iMenuProperties[id] = 0;
		...
		default:
		{
			...
			switch (g_iMenu[id])
			{
				...
				case STACK, SLAY, INVINCIBLE, NOCLIP, SPEED, UNAMMO: client_cmd(id, g_szMenuCommands[g_iMenu[id]], szName, g_iMenuProperties[id]);
...
DisplayPlayer2Menu(id, iPos, iMenu)
{
	...
	switch (iMenu)
	{
		...
		case INVINCIBLE, NOCLIP, SPEED, UNAMMO:
		{
			format(szKey, charsmax(szKey), "AMXSUPER_INVINCIBLE%d", g_iMenuProperties[id]);
			iLen += format(szMenuBody[iLen], 1023-iLen, "\r8. \w%L", id, szKey);
		}
		case BADAIM:
		{
			format(szKey, charsmax(szKey), "AMXSUPER_%s", (g_iMenuProperties[id] < 2) ? (g_iMenuProperties[id]) ? "INVINCIBLE0" : "INVINCIBLE1" : "MINS");
